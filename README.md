...
- **Rare Goods Finder** (click "Rares" in the panel) — the nearest rare commodities to your current system, sorted by distance: origin system and station, pad size, cost, PowerPlay eligibility, and a general legality note (which superpowers/government types restrict it — not evaluated against a specific destination, so always verify in-game before trading). "Show nearest" controls how many to list. Double-click a row to open that system's page on Inara. Shows "Awaiting system data…" until EDPPMT has seen a `FSDJump`/`Location` event this run. The 141-entry dataset (origin, station, pad, legality, PowerPlay eligibility, and galactic coordinates) is bundled with the plugin — coordinates were resolved once via EDSM and baked in, so this never makes a network call at runtime (rare-good locations don't move). See [`docs/ATTRIBUTIONS.md`](docs/ATTRIBUTIONS.md) for where the dataset comes from.
