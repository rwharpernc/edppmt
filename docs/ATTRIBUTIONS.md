...
| [EDMCOverlay](https://github.com/inorton/EDMCOverlay) | **Not bundled or installed by EDPPMT.** `plugin/overlay.py` is EDPPMT's own client, written against EDMCOverlay's published local-TCP JSON protocol (confirmed against its own `edmcoverlay.py`, plus the server-side `Graphic`/`GraphicType`/`VectorPoint`/`OverlayRenderer`/`OverlayJsonServer`/`InternalGraphic` C# sources — the last two are what confirmed a connection's graphics are deleted on disconnect independent of `ttl`, the basis for `OverlayClient` holding one persistent connection rather than one per send) — it's an optional, separately-installed tool the Interdiction Warning and Landing features draw through if the user has it running. | No license declared in the upstream repo at time of writing — EDPPMT's own client code (`overlay.py`) is original, written to the protocol, not copied from it |
| [EDMCModernOverlay](https://github.com/SweetJonnySauce/EDMCModernOverlay) | **Not bundled or installed by EDPPMT.** A newer, backwards-compatible community replacement for EDMCOverlay that `overlay.py`'s existing protocol client already works against unmodified. Its `docs/wiki/send_shape-API.md` confirmed that `"#AARRGGBB"` alpha-channel fills (used for the translucent card background behind Interdiction Warning/Landing) work identically on both, and documented an optional `thickness` field (`send_shape`'s crisper explicit border width) as a safe-to-send, silently-ignored-on-classic extension — `overlay.py`'s `send_shape` passes it through only when given. `overlay.register_modern_overlay_group()` additionally calls its public `overlay_plugin.overlay_api.define_plugin_group()` (documented in `docs/wiki/define_plugin_group-API.md`) to register EDPPMT's payload ids as Plugin Groups — needed for EDMCModernOverlay's per-group Fill-mode vector scaling (its own `Concepts.md`: "automatic scaling of Payloads (especially vector images)") to keep Landing's multi-piece pad diagram rendering as one cohesive unit instead of each shape scaling independently around its own tiny bounding box. A plain `ImportError` (classic EDMCOverlay, or no overlay plugin installed) makes this a no-op. | [GPL-3.0](https://github.com/SweetJonnySauce/EDMCModernOverlay/blob/main/LICENSE) |
...
|--------|---------------|
| [Frontier Developments](https://www.frontier.co.uk/) — *Elite Dangerous* | Player Journal events (`PowerplayMerits`, `Powerplay`, `PowerplayJoin`, `PowerplayLeave`, `PowerplayDefect`, `PowerplayRank`, `FSDJump`, `Location`, `Docked`, `Undocked`, `DockingRequested`, `DockingGranted`, `DockingDenied`, `DockingTimeout`, `DockingCancelled`, `LoadGame`, `ReceiveText`, `Interdicted`, `EscapeInterdiction`), plus `Status.json`'s `Flags` field (bit 23, "Being Interdicted") |
...
| Community rare-goods references (various Powerplay/trading guides) | The core `plugin/rare_goods.json` dataset itself (origin system, station, pad size, cost, legality restrictions, PowerPlay eligibility for 141 rare goods) was compiled by the plugin author from public game data and community references, then adapted for EDPPMT's own nearest-rares-to-current-system use case. |
...
| [bgol/LandingPad](https://github.com/bgol/LandingPad) (a third-party EDMC plugin, GPL-2.0 — read directly to verify this) | `plugin/landing.py` is an original implementation by this plugin's own author, not a port of this plugin. bgol/LandingPad enters the lineage in one specific place only: its `lpads/starport.py` is the original reference for the starport pad-index numbering (`pad_list`/`dodecagon`/`pad_sectors`, mirrored in `landing.py` as `_PAD_LIST`/`_DODECAGON`/`_PAD_SECTORS`) — a table dictated by the real game's actual station layout (any correct implementation reproduces the same 15 entries; there's no creative choice being copied) rather than expression original to bgol/LandingPad. Everything else in `landing.py` — the docking state machine, status text, auto-hide timing, and the EDMCOverlay send-vector/send-shape rendering — was written independently, without reference to bgol/LandingPad's own source, which reads and renders quite differently in the details even though it targets the same EDMCOverlay protocol (e.g. its pad marker is an unlabeled dot/small-rect cluster with no number shown on it, versus `landing.py`'s number-labeled marker; it uses a persistently-open socket, same conclusion EDPPMT separately arrived at fixing its own connection-per-message bug — see the EDMCOverlay row above). |

Auto-Honk, Interdiction Warning, and Landing also build on the plugin author's own prior, unreleased development work (design and some implementation details, not third-party code) — not listed here as a named/linked project since it isn't public yet.
