...
| EDDDT (a sibling project, an Electron/TypeScript app — `src/main/auto-honk/`, `src/main/input/`, `src/main/journal/binds.ts`, `src/main/interdiction/tracker.ts`, `src/shared/interdiction.ts`, `src/shared/docking.ts`, `src/main/journal/session-state.ts`, `src/renderer/src/components/PadDiagram.tsx`, `src/renderer/src/overlay-widgets/LandingOverlayWidget.tsx`) | Auto-Honk's implementation is a straight port from EDDDT: the binds-file lookup, the ED key-token-to-Windows-virtual-key `KEY_MAP` (built from a real `Custom.binds` file), and the Win32 key-injection approach (including the "fake Alt tap" foreground-lock workaround), re-implemented in Python via `ctypes` in place of EDDDT's Node/PowerShell approach. **Interdiction Warning** (`plugin/interdiction.py`) is likewise a direct port of EDDDT's detection state machine and chat-taunt phrase list (`CHAT_THREAT_PATTERNS`, itself originally from the author's ED-obs-app) — re-implemented without Tk/EventEmitter, drawn via EDMCOverlay instead of EDDDT's own Electron overlay window since EDPPMT has no equivalent renderer of its own. **Landing Pad** (`plugin/landing.py`) is likewise a direct port of EDDDT's docking state machine, status/pad-diagram-gating derivation, and pad-diagram geometry (dodecagon-shell math for starports, pad-rect layout for fleet carriers) — re-implemented via EDMCOverlay's `"vect"`/`"rect"` shape primitives in place of EDDDT's SVG rendering. |
...
| [bgol/LandingPad](https://github.com/bgol/LandingPad) (a third-party EDMC plugin, reviewed via EDDDT's own citation, not directly) | The original source (per EDDDT's `docking.ts`/`PadDiagram.tsx` comments, themselves via the author's ED-obs-app) for the starport dodecagon pad-index math and the fleet-carrier pad-grid layout that `plugin/landing.py`'s diagram geometry constants (`_DODECAGON`, `_PAD_LIST`, `_PAD_SECTORS`, `_fleetcarrier_pad_rects`) trace back to. |
